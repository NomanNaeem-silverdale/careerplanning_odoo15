...
class CareerMl(models.Model):
    ...
    _inherit = ['employee.profile' , 'mail.thread', 'mail.activity.mixin']
    ...
    company_preferencee = fields.Many2one('company.settle',string='Type of Company Want to Settle In')
    ...
    suggested_job_role = fields.Char(string='suggested_job_role',readonly=1)

    def action_prediction(self):
        with open('/home/noman/Odoo15v2/odoo_15/custom_addons/career_planing/static/model1.pkl', 'rb') as file:
        ...
        for rec in self:
            data = {
                ...
                "interested career area": rec.interested_career_area.name.lower(),  # lowercase the name
                "Type of company want to settle in?": rec.company_preferencee.type,  # typo corrected
